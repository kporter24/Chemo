...
#Subset the data to remove rows that include POOLED samples
Chemo_neg <- Chemo_neg[, 26:84]
Chemo_no_pooled <- Chemo_neg_labels[Chemo_neg_labels$CHEMO_PRE_POST != "POOLED", ]

#Changing the data set from a frame to a matrix:
Chemo_neg <- as.matrix(Chemo_neg)

#Get the samples to overlap between Chemo_neg and Chemo_no_pooled
common.ids <- intersect(colnames(Chemo_neg), rownames(Chemo_no_pooled))
common.ids <- sort(common.ids)
Chemo_neg <- Chemo_neg[, common.ids]
Chemo_no_pooled <- Chemo_no_pooled[common.ids,]
...
#Merge the data from 'data_full' and 'Chemo_no_pooled' to include all pertinent info. in a single table
#This also makes it easier to plot using ggplot2
data_merged <- merge(data_full, Chemo_no_pooled, by=0)

ggplot(data_merged, aes(PC1, PC2, group = patient_id)) + geom_point(aes(color=CHEMO_PRE_POST), size = 2.5) + geom_line(size=0.1) + scale_color_manual(values = c("PRE_CHEMO" = "coral", "POST_CHEMO" = "cyan3")) + theme_bw() + theme(panel.grid.major = element_blank())
#Can also add panel.grid.minor = element_blank() to get rid of ALL gridlines

ggplot(data_merged, aes(PC1, PC2, group = patient_id)) + geom_point(aes(color=HCT_PRE_POST), size = 2.5) + geom_line(size=0.1) + scale_color_manual(values = c("PRE_HCT" = "coral", "POST_HCT" = "cyan3")) + theme_bw()+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())

ggplot(data_merged, aes(PC1, PC2, group = patient_id)) + geom_point(aes(color=Patient_status), size = 2.5) + geom_line(size=0.1) + scale_color_manual(values = c("Pre_chemo" = "darkorchid1", "Post_chemo_pre_HCT" = "deepskyblue", "Post_HCT" = "olivedrab3")) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
